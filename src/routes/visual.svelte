...
<section>
	...
	<p>
		...
		percentage of a full-time job that they undertake (subject to minor variances related to public
		holidays, rounding, etc).
	</p>
	<p>
		In other words, an employee who works 2.5 days a week (50%) should receive approximately 50% of
		the holiday benefits, whether those are calculated in days, or in hours. Likewise for 25% and
		75% part-time workers, and everyone in-between.
	</p>
	<p>
		An extension of this concept would suggest that a part-time worker should earn the same holiday
		benefits as a full-time worker clocking the same hours. In other words, if a full-time worker
		earns a single holiday "hour" after working 9.5 hours, then a part-time worker should expect the
		same exchange rate regardless of whether that worker works 10 hours a week, or 30.
	</p>
...
<section>
	...
	<ul>
		<li>
			A "naive" calculation - As above this essentially plots the part-time hours percentage on the
	...
	<p>
		As you can see, the conversion based on 7.5 hour days closely tracks the green line which shows
		the expectation that they receive benefits in proportion to the hours worked.
	</p>
	<p>
		...
		employee should receive. If we factor it in again when converting those <em>already reduced</em>
...
<section>
	...
	<p>
		This calculation is applied to both the 7.5 hour conversion and the part-time holiday conversion
		described in the previous chart. It is simply another way to visualise the same deviation
		highlighted above.
	</p>
	...
	<p>
		As you can see, a full-time worker earns a single holiday hour for every 9.5 hours of work. And
		the dataset (in pink) which considers a holiday to be 7.5 hours regardless of hours worked
		sticks closely in line with that. There are small variances which are accounted for by the fact
		that part-time holidays are rounded up to the nearest half day.
	</p>

	<p>
		The dataset (in yellow) which shows the value derived from assuming a part-time holiday is less
		than 7.5 hours, deviates extremely from the full-time exchange rate. Depending on the length of
		employment, and the number of hours worked per week, a part-time worker could have to work up to
		50 hours to earn a single hour of holiday.
	</p>
</section>
